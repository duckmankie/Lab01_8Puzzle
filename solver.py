...
def ucs(start_board):
    ...
    solve_time = end_time - start_time
    frontier_nodes = len(heap)
    return [], nodes_expanded, 0, solve_time, frontier_nodes  # không tìm thấy lời giải
...
def bidirectional_search(start_board):
    ...
    if start_key == goal_key:
        return [], 0, 0, 0.0, 0  # Đã là trạng thái đích

    # forward và backward queue
            ...
            for dr, dc in MOVES:
                ...
                if 0 <= nr < 3 and 0 <= nc < 3:
                    ...
                    if new_key not in backward_visited:
                        backward_visited[new_key] = path + [(br, bc)]  # Lưu chiều ngược
